...
name: afk
description: >-
  Enter away mode when the captain invokes `/afk`, says they are going AFK, `state/.afk` exists, an incoming message starts with `FM_INJECT_MARK`, or a `state/.subsuper-*` marker exists.
  Exit when an unmarked captain message returns.
user-invocable: true
metadata:
  internal: true
---
