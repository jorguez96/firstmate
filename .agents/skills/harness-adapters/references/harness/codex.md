...
| Effort flag | `-c 'model_reasoning_effort="<level>"'`; `max` is passed only when the selected model's installed `~/.codex/models_cache.json` entry lists it in `supported_reasoning_levels`, verified on codex-cli 0.152.0. |
