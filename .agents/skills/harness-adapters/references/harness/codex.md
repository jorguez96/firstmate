...
| Effort flag | `-c 'model_reasoning_effort="<level>"'`; `max` is passed only when the selected model's installed `${CODEX_HOME:-~/.codex}/models_cache.json` entry lists it in `supported_reasoning_levels` (verified on codex-cli 0.152.0; the 0.153.4 catalog advertises `max` for `gpt-5.6-luna`, which Firstmate passes for that model). |
