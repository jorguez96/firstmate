...
The branch's runtime files stay with their format and lifecycle owners.
`bin/fm-branch-outcome.sh` owns the outcome store, the Pi extension owns the branch session (docs/pi-supervision-branch.md), and `bin/fm-lease-lib.sh` owns the supervision leases.
